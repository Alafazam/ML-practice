...
def kcluster(rows,distance=pearson,k=4):
	...
	ranges=[(min([row[i] for row in rows]),max([row[i] for row in rows])) for i in range(len(rows[1]))]
	
	# Create k randomly placed centroids
	clusters=[[random.random( )*(ranges[i][1]-ranges[i][0])+ranges[i][0] for i in range(len(rows[1]))] for j in range(k)]
